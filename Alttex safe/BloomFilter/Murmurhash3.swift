...
public class Murmurhash3 {
    ...
    //MurmurHash3 32 bits.
    public static func doHash32(key: UnsafePointer<UInt8>, maxBytes: Int, seed: UInt32 = 0) -> UInt32 {
        ...
        let chunks = UnsafePointer(key).withMemoryRebound(to: UInt32.self, capacity: l){
            UnsafeBufferPointer(start: $0, count: l)
        }
    ...
    public static func hash32Bytes(key: [UInt8] ,
                                   seed: UInt32 = 0) -> UInt32 {
        var a = key
        return doHash32(key: &a, maxBytes: a.count, seed: seed)
    }
    ...
    public static func doHash128(key: UnsafePointer<UInt8>, maxBytes: Int, seed: UInt64 = 0) -> (h1: UInt64, h2: UInt64) {
        ...
        let blocks = UnsafePointer(key).withMemoryRebound(to: UInt64.self, capacity: nblocks){
            UnsafeBufferPointer(start: $0, count: nblocks)
        }
    ...
    public static func hash128Bytes(key: [UInt8],
                                    seed: UInt64 = 0) -> (h1: UInt64, h2: UInt64) {
        var a = key
        return doHash128(key: &a, maxBytes: a.count, seed: seed)
    }
