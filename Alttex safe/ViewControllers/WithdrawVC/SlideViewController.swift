...
class SlideViewController: UIViewController, BITransactionHistoryViewDelegate, BISendTopViewDelegate, BIQRCodeReadViewControllerDelegate, BIPayViewControllerDelegate, UIScrollViewDelegate, CFControllerDelegate, BIReceiveTopViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.view.backgroundColor = UIColor.rbg(r: 24, g: 24, b: 24)
    ...
    func setupContentView() {
        ...
        let contentRect = CGRect(x: 0, y: 0, width: size.width * 2, height: size.height)
        ...
        txHistoryView.backgroundColor = UIColor.rbg(r: 24, g: 24, b: 24)
...
//        contentView.addSubview(sendTopView)
        //size dot
        let receiveTopView = BIReceiveTopView(frame: CGRect(x: size.width * 1, y: 0, width: size.width, height: size.height))
        receiveTopView.delegate = self
        receiveTopView.backgroundColor = UIColor.rbg(r: 24, g: 24, b: 24)
        contentView.addSubview(receiveTopView)
        
        let statusViewHeight = size.height / 3
        ...
        topStatusView.setupStatusLabel(text: String(balanceInBTC) + "BTC")
    ...
    func transactionPassedToNode() {
        ...
        txSentTimer = Timer.scheduledTimer(timeInterval: timeout, target: self, selector: #selector(SlideViewController.txSuccessfullySent(_:)), userInfo: nil, repeats: false)
    }
