...
class BIPayViewController: UIViewController {
    ...
    @IBAction func send(_ sender: Any) {
        ...
        if addressTextField.text == "" || amountTextField.text == "" {
            self.displayAlert(title: NSLocalizedString("Invalid Input", comment: ""), message: NSLocalizedString("Address or amount is empty.", comment: ""))
            //print("Address or amount is empty.")
            return
        }
        
        guard let addressStr = addressTextField.text else {
            self.displayAlert(title: NSLocalizedString("Invalid Input", comment: ""), message: NSLocalizedString("Address  is empty.", comment: ""))
        ...
        guard let amountStr = amountTextField.text else {
            self.displayAlert(title: NSLocalizedString("Invalid Input", comment: ""), message: NSLocalizedString("Amount is empty.", comment: ""))
            //print("Amount is nil")
            return
        }
        
        guard let type = addressStr.determinOutputScriptTypeWithAddress() else {
             self.displayAlert(title: NSLocalizedString("Invalid Input", comment: ""), message: NSLocalizedString("Could not determin address type", comment: ""))
        ...
        if let pubkeyHash = addressStr.publicAddressToPubKeyHash(prefix) {
            ...
            self.displayAlert(title: nil, message: NSLocalizedString("Invalid Address", comment: ""))
    ...
    private func propagateTx(txConstructor: TransactionDBConstructor) {
        if let tx = txConstructor.transaction {
            ...
            self.displayAlert(title: NSLocalizedString("Error", comment: ""), message: NSLocalizedString("Unable to build transaction. Insufficient amount.", comment: ""))
        }
    }
